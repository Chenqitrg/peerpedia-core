...
**日期:** 2026-06-25 – 2026-06-26（三轮测试，含联网/server/REPL/双用户场景）
**分支:** main
**测试轮次:** 三轮。第一轮基础 CLI，第二轮联网 + server，第三轮双用户 + 邀请/评审/社交全链路。
**测试方法:** 以新用户身份，从零开始使用 CLI 走完完整工作流，同时测试边缘场景和错误路径。
**环境:** macOS, Python 3.14, 干净 `~/.peerpedia/` 数据目录。
...
| 维度 | 评分 | 说明 |
|------|------|------|
| 首次上手体验 | 3/10 | 🔴 创建文章不知道 ID，list 看不到自己的文章 |
| 命令一致性 | 4/10 | 🔴 不同命令对同一 ID 的解析结果不同 |
| 错误信息质量 | 6/10 | ⚠️ 部分清晰，部分对普通用户无意义 |
| P2P 同步可用性 | 2/10 | 🔴 sync pull / social 端点认证链路断裂 |
| 非交互环境 | 2/10 | 🔴 强制弹 vim，无 TTY 检测 |
| 输出信息密度 | 4/10 | 🔴 关键信息（ID）缺失，反馈太弱 |
| **综合** | **3.5/10** | 🔴 |
...
| 优先级 | 问题 | 影响 |
|--------|------|------|
| **P0** | REPL 启动崩溃（#17） | 人类入口不可用 |
| **P0** | 创建文章不显示 ID（#1） | ✅ 已修复 |
| **P0** | `article list` 默认看不到 draft（#2） | ✅ 已修复 |
| **P0** | sync pull 401（#6） | ✅ 已修复 |
| **P0** | `review invite` NameError（#13） | ✅ 已修复 |
| **P0** | `review rate` NameError → AttributeError（#14→#19） | 🔴 regression，半修不修 |
| **P0** | `args.article_id` 6 个 handler 未解析（#18） | 🆕 系统性缺陷 |
| **P1** | ID 解析跨命令不一致（#4） | ✅ #18 的根因 |
| **P1** | Server 要求完整 UUID / CLI 传短 ID（#5/#7） | ✅ 仍存在（同 #18） |
